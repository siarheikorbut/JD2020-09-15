package by.it.kolesnikov.jd01_01;

/*
напишите тело main программы которая выводит на экран 5 строк
"Я начинаю изучать Java!"

Для ручной проверки запустите программу Ctrl+Shift+F10
Для автоматической проверки откройте и запустите класс Test_jd01_01
*/

class TaskA2 {
    public static void main(String[] args) {
        for(int i=0; i<5; i++) {     //цикл, переменная i возрастает от 0 до 5
            System.out.println("Я начинаю изучать Java!");   //тело цикла, System.out.println выводит значение взятое в скобки и повторяет его 5 раз
        }
    }
}
